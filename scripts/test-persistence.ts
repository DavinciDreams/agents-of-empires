...
async function testPersistence() {
  ...
  try {
    ...
    const checkpoint = await resultsPersistence.getCheckpointState('checkpoint-001');
    if (checkpoint) {
    ...
    const logs = await resultsPersistence.getExecutionLogs('exec-001');
    ...
    const traces = await resultsPersistence.getExecutionTraces('exec-001');
